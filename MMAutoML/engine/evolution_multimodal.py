...
from lib.datasets import build_dataset, collate_fn
from lib import utils
from engine.supernet_engine import evaluate
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir):
        ...
        self.evo_checkpoint_path = args.evo_resume
        self.candidates = []
        self.top_accuracies = []
        self.cand_params = []
    ...
    def save_checkpoint(self):
        ...
        ckpts_dir = os.path.join(self.output_dir, 'checkpoints')
        os.makedirs(ckpts_dir, exist_ok=True)

        checkpoint_path = os.path.join(ckpts_dir, "checkpoint-{}.pth.tar".format(self.epoch))
        torch.save(info, checkpoint_path)
        print('save checkpoint to', checkpoint_path)

    def load_checkpoint(self):
        if not os.path.exists(self.evo_checkpoint_path):
            return False
        info = torch.load(self.evo_checkpoint_path)
        ...
        print('load checkpoint from', self.evo_checkpoint_path)
        return True
    ...
    def is_legal(self, cand):
        ...
        eval_stats = evaluate(self.val_loader, self.model, self.device, amp=self.args.amp, mode=self.args.mode,
                              retrain_config=sampled_config, args=self.args)
        test_stats = evaluate(self.test_loader, self.model, self.device, amp=self.args.amp, mode=self.args.mode,
                              retrain_config=sampled_config, args=self.args)

        info['f1'] = eval_stats['f1_score']
        info['test_f1'] = test_stats['f1_score']
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: self.vis_dict[x]['f1'])
            self.update_top_k(
                self.candidates, k=50, key=lambda x: self.vis_dict[x]['f1'])

            print('epoch = {} : top {} result'.format(
                self.epoch, len(self.keep_top_k[50])))
            tmp_accuracy = []
            for i, cand in enumerate(self.keep_top_k[50]):
                print('No.{} {} Top-1 val f1 = {}, Top-1 test f1 = {}, params = {}'.format(
                    i + 1, cand, self.vis_dict[cand]['f1'], self.vis_dict[cand]['test_f1'],
                    self.vis_dict[cand]['params']))
                tmp_accuracy.append(self.vis_dict[cand]['f1'])
            ...
            print("Saving top architectures...")
            self.save_top_architectures(self.epoch)

        best_architecture = self.keep_top_k[50][0]
        return best_architecture

    def save_top_architectures(self, epoch):
        """Save the best results found throughout the evolution process"""
        results = {}
        for i, cand in enumerate(self.keep_top_k[50]):
            results[f'No.{i + 1}'] = {
                'candidate': str(cand),
                'top1_val_f1': self.vis_dict[cand]['f1'],
                'top1_test_f1': self.vis_dict[cand]['test_f1'],
                'params': self.vis_dict[cand]['params']
            }

        arch_dir = os.path.join(self.output_dir, 'architectures')
        os.makedirs(arch_dir, exist_ok=True)

        json_path = os.path.join(arch_dir, 'arch_{}.json'.format(epoch))
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f'Best {len(results)} architectures saved to {json_path}')
...
def get_args_parser():
    ...
    parser.add_argument('--param_limits', type=float, default=23)
    parser.add_argument('--min_param_limits', type=float, default=18)

    # config file
    parser.add_argument('--cfg', help='experiment configure file name',
                        default='./experiments/supernet/supernet-T.yaml', type=str)
    ...
    parser.add_argument('--data-path', default='./data/', type=str,
                        help='dataset path')
    parser.add_argument('--dataset_name', default='MMIMDB',
                        choices=['CIFAR10', 'IMNET', 'INAT', 'INAT19', 'MMIMDB', 'mmimdb'],
    ...
    parser.add_argument('--evo-resume', default='', help='evolution_resume from checkpoint')
...
def evolution_search_experiment(args):
    ...
    dataset_val, args.nb_classes = build_dataset(is_train=False, args=args, folder_name="dev")
    dataset_test, _ = build_dataset(is_train=False, args=args, folder_name="test")
    ...
    data_loader_test = torch.utils.data.DataLoader(
        ...
        pin_memory=args.pin_mem, drop_last=False, collate_fn=collate
    )
...
if __name__ == '__main__':
    ...
    utils.init_distributed_mode(args)
    evolution_search_experiment(args)
