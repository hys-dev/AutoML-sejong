...
from engine.supernet_engine import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    parser.add_argument('--lr_scheduler', default='cosine', type=str, metavar='SCHEDULER',
    ...
    parser.add_argument('--min_learning_rate', type=float, default=1e-5, metavar='LR',
    ...
    parser.add_argument('--data-path', default='./data/', type=str,
                        help='dataset path')
    parser.add_argument('--dataset_name', default='MMIMDB', choices=['CIFAR10', 'MMIMDB', 'mmimdb'],
...
def nas_search_experiment(args):
    ...
    dataset_train, args.nb_classes = build_dataset(is_train=True, args=args, folder_name="train")
    dataset_val, _ = build_dataset(is_train=False, args=args, folder_name="dev")
    ...
    if args.distributed:
        ...
        # The unique GPU ID of each process
    ...
    exp_key = os.path.basename(output_dir)

    linear_scaled_lr = args.learning_rate * args.batch_size * utils.get_world_size() / 512.0
    args.lr = linear_scaled_lr

    args.min_lr = args.min_learning_rate
    args.epochs = args.max_epochs
    args.opt = args.optimizer
    args.sched = args.lr_scheduler
    ...
    # save logs
    csv_writer, metrics_file = None, None
    if args.output_dir and utils.is_main_process():
        metrics_file = open(output_dir / "metrics.csv", "w", newline='')
        csv_writer = csv.writer(metrics_file)
        csv_writer.writerow([
            "train_loss", "train_f1_score", "epoch", "step",
            "test_loss", "test_f1_score", "lr", "n_parameters"
        ])
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, mode=args.mode, retrain_config=retrain_config, args=args)
        if args.task_type == "multilabel":
            print(f"F1_Score of the network on the {len(dataset_val)} test images: {test_stats['f1_score']:.1f}%")
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            csv_writer=csv_writer, metrics_file=metrics_file, args=args,
        )
        ...
        test_stats = evaluate(data_loader_val, model, device, amp=args.amp, choices=choices, mode=args.mode,
                              retrain_config=retrain_config, args=args)
        if args.task_type == "multilabel":
            print(f"F1_Score of the network on the {len(dataset_val)} test images: {test_stats['f1_score']:.1f}%")
            old_max = max_metric
            max_metric = max(max_metric, test_stats["f1_score"])
            print(f'Max F1_Score: {max_metric:.2f}%')
        else:
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            old_max = max_metric
            max_metric = max(max_metric, test_stats["acc1"])
            print(f'Max accuracy: {max_metric:.2f}%')

        if max_metric > old_max:
            if args.output_dir:
                ...
                for best_model_path in best_model_paths:
                    ...
                    print(f"New best model saved (epoch {epoch}, Max metric={max_metric:.2f}%)")

        if csv_writer is not None and utils.is_main_process():
            step = (epoch + 1) * len(data_loader_train)
            csv_writer.writerow(
                ["", "", epoch, step, test_stats['loss'], test_stats['f1_score'], train_stats['lr'], n_parameters])
            metrics_file.flush()
