...
from model.module.text_embedding_super import TextembedSuper
...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.text_embed_super = TextembedSuper(hidden_size=embed_dim, args=self.args)
        self.gp = gp
    ...
    def set_sample_config(self, config: dict):
        ...
        # text_embed
        self.text_embed_super.set_sample_config(self.sample_embed_dim[0])
    ...
    def get_complexity(self, sequence_length):
        ...
        return total_flops
    def forward_features(self, text, segment, x):
        # image
        B = x.shape[0]
        x = self.patch_embed_super(x)
        cls_tokens = self.cls_token[..., :self.sample_embed_dim[0]].expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1) # B x N + 1 x embed_dim
        if self.abs_pos:
            x = x + self.pos_embed[..., :self.sample_embed_dim[0]]

        x = F.dropout(x, p=self.sample_dropout, training=self.training) # 16, 197, 216

        # txt
        text_embed_out = self.text_embed_super(text, segment)
        x = torch.cat((x, text_embed_out), dim=1)
    ...
    def forward(self, text, segment, x):
        # text : torch.Size([16, 376])  segment : torch.Size([16, 376])
        x = self.forward_features(text, segment, x)
        x = self.head(x)
        return x
